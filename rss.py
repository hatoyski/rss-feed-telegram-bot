...
def create_feed_checker(feed_url):
    def check_feed():
        ...
        if last_id_from_db == "*":
            ...
            try:
                if "RARBG" in first_entry.link or "Galaxy" in first_entry.link:
        ...
        for entry_num, entry in enumerate(FEED.entries):
            ...
            try:
                if "ION10" in entry.link or "720" in entry.link or "1080" in entry.link or "265" in entry.link or "264" in entry.link:
                    app.send_message(log_channel, message)
                else:
